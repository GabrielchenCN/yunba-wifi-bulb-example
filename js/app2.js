...
var MicrophoneAudioSource = function() {
    ...
    //全局变量监控声音是否调用,配置别名
    var frevolObj={
        "Sarah":{"r":105,"g":"","b":255,"volume":0},
        "Kathie":{"r":255,"g":"","b":255,"volume":0},
        "Christina":{"r":204,"g":"","b":154,"volume":0},
        "Frieda":{"r":204,"g":"","b":102,"volume":0},
        "Jamie":{"r":255,"g":"","b":51,"volume":0}
        // "Sarah":{"r":0,"g":255,"b":153,"volume":0},
        // "Kathie":{"r":0,"g":255,"b":30,"volume":0},
        // "Christina":{"r":255,"g":255,"b":255,"volume":0},
        // "Frieda":{"r":255,"g":255,"b":0,"volume":0},
        // "Jamie":{"r":255,"g":51,"b":0,"volume":0}
        // "f":{"r":1,"g":"","b":1,"volume":0}
    ...
    var sampleAudioStream = function() {
        ...
        //console.log(frequencyVolume(100,120));
        //setInterval(yunba("yunbaio",'1','0','0',frequencyVolume(50,70)),1000);
        // 声音判断逻辑：
        // 变量  frevolArr：保存某一频段的声音大小 
        // 方法  frequencyVolume；获得某一频段的声音大小（区间0-128，由fftsize确定）
        // 方法  yunba ：根据别名对某一灯泡传入rgb数据和frevol声音大小
        // 方法  closebulb 当所有频率声音大小self.volume小于300时，发送关闭灯泡请求 

        frevolObj.Sarah.volume += frequencyVolume(0,20);
        frevolObj.Kathie.volume  += frequencyVolume(21,40);
        frevolObj.Christina.volume  += frequencyVolume(41,60);
        frevolObj.Frieda.volume  += frequencyVolume(61,80);
        frevolObj.Jamie.volume  += frequencyVolume(81,100);


       for(var key in frevolObj)
        {   
            var tmp =frevolObj[key].volume;
            var r  =frevolObj[key].r;
            var g  =(Number(frevolObj[key].volume)/100).toFixed(0);
            var b  = frevolObj[key].b;
            //调节tmp需要调节checkcv函数
            if (tmp>4000&&num<=6) {

            yunba(key,r,g,b,tmp);
            console.log(key+"---"+tmp+":rgb("+r+","+g+","+b+")");  
            frevolObj[key].volume=0;
            if (num==0) {
                num =25;
            };
...
//某频率段的音量大小
    var frequencyVolume=function(start,end){
           var fv = 0;
           for (var i = start; i<end;i++) {
            //console.log(fv);
            fv += self.streamData[i];
        };
...
//发送到云巴服务器yunba 
var yunba = function(alias,cr,cg,cb,volume){
     // if (volume==0) {return };
    var checkcv=function(colour){
           if (colour<=11111) {
               return (colour/128).toFixed(0);
            }
            else  return 87;
    ...
    var config ={
        ...
        config.alias=alias;
        config.p=1000;
        config.red=checkcv(volume)*cr;
        config.green=cg*checkcv(volume);
        config.blue=cb*checkcv(volume);
        

var urlproxy = '/html/yunba-bulb-settings/html/getbm?method=publish_to_alias&appkey=56556dd4f085fc471efe0688&seckey=sec-uTYMY37JTlH5hEmsvmgO8FkZYwkkPA45VAuDifUeQIsh4enS&alias='+config.alias+'&msg={"p":'+config.p+',"r":'+config.red+',"g":'+config.green+',"b":'+config.blue+'}';
...
$.ajax({
  ...
  success: function(res){
    ...
    if(res.status=="0"){
        console.log("got it r："+config.red+"g:"+config.green+"b:"+config.blue);
...
//关闭灯泡
    var closebulb=function(){
   
       if (self.volume<300) {
        setTimeout(yunba("yunbaio",'1','0','0',0),1000) 
          console.log(self.volume);
         };
}
...
var Visualizer = function() {
    ...
    Polygon.prototype.drawPolygon = function() {
        ...
        //console.log(audioSource.streamData[bucket]/255);
...
window.onload = function init() {
 ...
 var audioSource = new MicrophoneAudioSource(player);
